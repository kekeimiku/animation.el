...
static MTKView *shiftGlitchView = nil;
...
@interface ShiftGlitchView : MTKView <MTKViewDelegate>
...
@implementation ShiftGlitchView
...
- (BOOL)setupMetal {
  ...
  self.captureQueue = dispatch_queue_create("com.shiftglitch.capture", DISPATCH_QUEUE_SERIAL);
  self.isCapturing = NO;
  ...
  NSString *metallibPath = [[libPath stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"shift-glitch.metallib"];
  ...
  id<MTLFunction> fragmentFunction = [library newFunctionWithName:@"shift_glitch_effect"];
...
static void removeShiftGlitchOverlay() {
  if (shiftGlitchView) {
    [shiftGlitchView removeFromSuperview];
    shiftGlitchView = nil;
  }
}

static void createShiftGlitchOverlay() {
  if (shiftGlitchView) return;

  NSView *contentView = [NSApp mainWindow].contentView;
  shiftGlitchView = [[ShiftGlitchView alloc] initWithFrame:contentView.bounds device:MTLCreateSystemDefaultDevice()];

  if (![(ShiftGlitchView *)shiftGlitchView setupMetal]) {
    shiftGlitchView = nil;
    return;
  }

  ShiftGlitchView *glitchView = (ShiftGlitchView *)shiftGlitchView;
  ...
  glitchView.preferredFramesPerSecond = 60;
...
static emacs_value shift_glitch_render_emacs(emacs_env *env, ptrdiff_t nargs, emacs_value *args, void *data) {
  dispatch_async(dispatch_get_main_queue(), ^{
    createShiftGlitchOverlay();
  });

  return env->intern(env, "nil");
}

static emacs_value shift_glitch_cleanup_emacs(emacs_env *env, ptrdiff_t nargs, emacs_value *args, void *data) {
  dispatch_async(dispatch_get_main_queue(), ^{
    removeShiftGlitchOverlay();
  });
...
__attribute__((used, visibility("default"))) int emacs_module_init(struct emacs_runtime *runtime) {
  ...
  emacs_value render_args[] = {
      env->intern(env, "shift-glitch-render"),
      env->make_function(env, 0, 0, shift_glitch_render_emacs,
                         R"(Render RGB shift glitch effect using Metal shader.)", NULL)};
  env->funcall(env, defalias, 2, render_args);

  emacs_value cleanup_args[] = {
      env->intern(env, "shift-glitch-cleanup"),
      env->make_function(env, 0, 0, shift_glitch_cleanup_emacs,
                         R"(Stop and clean up RGB shift glitch effect.)", NULL)};
